...
parser.add_argument('-d', action="store", dest="directoryToScan", required=True, help='Directory to scan for Photo (sub directory included)')
...
def AddPhotoToES(fullPathFile):
	...
	if arguments.forceAllNote or not ("\"found\":true" in photoFromES.text) or (arguments.forceAllNoteExcept0 and not ("\"note\": 0" in photoFromES.text)):
	    ...
	    print( "      "+ md5file)
	    print( "      "+ dateFile)
	    print( "      "+ str(arguments.defaultNote))
	    payload = '{{\"fullpath\": "{0}", "dateFile": "{1}", "note": {2} }}'.format(fullPathFile.replace('\\', '\\\\'),dateFile,str(arguments.defaultNote))
	    insertRequest = requests.put(arguments.baseESurl+md5file, data=payload)
	    print( "      --> " + str(insertRequest.status_code))



for dossier, sous_dossiers, fichiers in os.walk(arguments.directoryToScan):
